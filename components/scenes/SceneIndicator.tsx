...
export function SceneIndicator({ currentIndex }: SceneIndicatorProps) {
  ...
  const soundScenes = SCENES.filter(
    (s) => s.soundId === currentScene.soundId
  );
  const positionInGroup = soundScenes.findIndex(
    ...
    <div className="fixed bottom-2 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3">
      ...
      <div className="flex gap-1.5">
        {soundScenes.map((scene, index) => (
          <div
            key={scene.id}
            className={`h-1.5 rounded-full transition-all duration-300 ${index === positionInGroup - 1
                ? "w-6 bg-white"
                : "w-1.5 bg-white/30"
              }`}
          />
